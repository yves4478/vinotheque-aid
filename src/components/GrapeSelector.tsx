...
export function GrapeSelector({ value, onChange, country, className }: GrapeSelectorProps) {
  ...
  return (
    <div ref={containerRef} className={cn("space-y-2", className)}>
      ...
      {mode === "single" && (
        <select
          value={grapeOptions.includes(value) ? value : ""}
          onChange={(event) => selectSingleGrape(event.target.value)}
          className="h-11 w-full rounded-xl border border-gray-200 bg-white px-3 text-sm outline-none transition-colors focus:border-primary/50 focus:ring-2 focus:ring-primary/10"
        >
          <option value="">{country ? `Rebsorte aus ${country} wählen…` : "Rebsorte wählen…"}</option>
          {grapeOptions.map((grape) => (
            <option key={grape} value={grape}>{grape}</option>
          ))}
          <option value={OTHER_GRAPE_OPTION}>Andere…</option>
        </select>
      )}
